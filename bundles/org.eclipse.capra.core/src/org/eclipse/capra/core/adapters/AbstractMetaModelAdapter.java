...
public abstract class AbstractMetaModelAdapter implements TraceMetaModelAdapter {
	...
	public List<Connection> getInternalElements(EObject element, EObject traceModel,
		...
		return getInternalElements(element, traceModel, selectedRelationshipTypes, false, 0, new ArrayList<>());
	}

	@SuppressWarnings("unchecked")
	private List<Connection> getInternalElements(EObject element, EObject traceModel,
		...
		for (Connection conn : directElements) {
			...
			for (EObject o : conn.getOrigins()) {
				...
				if (originHandler != null) {
					allElements.addAll(originHandler.getInternalLinks(o, selectedRelationshipTypes));
				}
			}
			...
			for (EObject o : conn.getTargets()) {
				...
				if (handler != null) {
					allElements.addAll(handler.getInternalLinks(o, selectedRelationshipTypes));
				}
			}
		}
		...
		if (directElements.isEmpty()) {
			...
			IArtifactHandler<?> handler = artifactHelper.getHandler(originalObject).orElseThrow();
			if (handler != null) {
				allElements.addAll(handler.getInternalLinks(element, selectedRelationshipTypes));
			}
