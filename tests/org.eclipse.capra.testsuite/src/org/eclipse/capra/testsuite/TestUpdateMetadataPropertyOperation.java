...
import org.eclipse.capra.ui.operations.UpdateMetadataPropertyOperation;
...
public class TestUpdateMetadataPropertyOperation {

	private static final String METADATA_PROPERTY = "Comment";
	private static final String COMMENT_TEXT = "Testeroni";
	...
	public void testUpdateTrace() throws CoreException, IOException {
		...
		// Create the trace via an operation
		...
		// Get the trace link
		Connection traceLink = traces.get(0);

		// Check that we have the property source and the property is found
		IPropertySource metadataPropertySource = Platform.getAdapterManager().getAdapter(traceLink,
				IPropertySource.class);
		assertNotNull(metadataPropertySource);
		IPropertyDescriptor[] descriptors = metadataPropertySource.getPropertyDescriptors();
		IPropertyDescriptor propertyDescriptor = Arrays.stream(descriptors)
				.filter(d -> d.getDisplayName().equals(METADATA_PROPERTY)).findFirst().orElse(null);
		assertNotNull(propertyDescriptor);
		Object featureId = propertyDescriptor.getId();

		// Create and run the operation
		UpdateMetadataPropertyOperation metadataUpdateOp = new UpdateMetadataPropertyOperation(traceLink,
				"Update trace metadata", METADATA_PROPERTY, COMMENT_TEXT);
		try {
			assertEquals(operationHistory.execute(metadataUpdateOp, null, adapter), Status.OK_STATUS);
		} catch (ExecutionException e) {
			fail("Could not update metadata: ExecutionException in operation");
		}

		// Check that everything is in place
		Object value = metadataPropertySource.getPropertyValue(featureId);
		assertNotNull(value);
		assertEquals(COMMENT_TEXT, value);
	}
